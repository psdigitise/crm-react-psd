...
// import { ChevronDown, ChevronUp, Phone, Loader2, Filter, Settings, X, ChevronLeft, ChevronRight, RefreshCcw, Disc } from 'lucide-react';
...
//         className={`w-full px-3 py-2 text-sm border rounded-lg transition-colors flex items-center justify-between ${theme === 'dark'
...
//       {isOpen && (
//         <div className={`absolute top-full left-0 mt-1 w-full rounded-lg shadow-lg z-20 p-2 ${theme === 'dark'
//           ? 'bg-dark-accent border border-purple-500/30'
//           : 'bg-white border border-gray-200'
//           }`}>
//           {options.map(option => (
//             <label key={option} className="flex items-center space-x-2 p-2 text-sm">
//               <input
//                 type="checkbox"
...
//   const exportToExcel = (data: Deal[], columns: Column[], filename: string = 'deals_export.xlsx') => {
//     const visibleColumns = columns.filter(col => col.visible);
//     const exportData = data.map(deal => {
//       const row: Record<string, any> = {};
//       visibleColumns.forEach(col => {
//         row[col.label] = deal[col.key as keyof Deal] ?? ''; // Use label as column title
//       });
//       return row;
//     });
...
//       const apiUrl = `http://103.214.132.20:8002/api/v2/document/CRM Deal?fields=["organization_name","website","no_of_employees","territory","annual_revenue","industry","salutation","first_name","last_name","email","mobile_no","gender","status","deal_owner","name"]&filters=${filters}&limit_page_length=1000&limit_start=0`;
...
//         organization_name: apiDeal.organization_name,
//         website: apiDeal.website,
//         no_of_employees: apiDeal.no_of_employees,
//         territory: apiDeal.territory,
//         annual_revenue: apiDeal.annual_revenue,
//         industry: apiDeal.industry,
//         salutation: apiDeal.salutation,
//         first_name: apiDeal.first_name,
//         last_name: apiDeal.last_name,
//         mobile_no: apiDeal.mobile_no,
...
//         status: Array.from(new Set(deals.map(deal => deal.status))),
//         territory: Array.from(new Set(deals.map(deal => deal.territory || '').filter(Boolean))),
//         industry: Array.from(new Set(deals.map(deal => deal.industry || '').filter(Boolean))),
//         assignedTo: Array.from(new Set(deals.map(deal => deal.assignedTo)))
//       };
...
//       (filters.territory.length === 0 || (item.territory && filters.territory.includes(item.territory))) &&
//       (filters.industry.length === 0 || (item.industry && filters.industry.includes(item.industry))) &&
//       (filters.assignedTo.length === 0 || filters.assignedTo.includes(item.assignedTo));
...
//         <table className={`rounded-lg shadow-sm border overflow-hidden ${theme === 'dark'
//           ? 'bg-custom-gradient border-transparent !rounded-none'
//           : 'bg-white border-gray-200'
//           }`}>
...
//                   }`}>
//                   <SortButton field={column.key as keyof Deal}>{column.label}</SortButton>
//                 </th>
//               ))}
//             </tr>
//           </thead>
...
//                 </td>
//                 {columns.filter(col => col.visible).map(column => (
//                   <td key={column.key} className={`px-6 py-4 text-sm font-semibold whitespace-nowrap ${theme === 'dark' ? 'text-white' : 'text-gray-900'
//                     }`}>
...
//       {/* Pagination Controls */}
//       {totalPages > 1 && (
//         <div className="flex flex-col sm:flex-row items-center justify-between gap-4 p-4 border-t border-gray-200 dark:border-purple-500/30">
...
//           <FaCircleDot className={`mr-1 ${statusColors[deal.status]}`} />
//           {deal.status}
//         </span>
//       );
//     case 'mobileNo':
