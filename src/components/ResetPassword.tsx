...
export default function PasswordResetPage() {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            // ✅ Use correct ERPNext API endpoint
            const response = await axios.post(
                'https://api.erpnext.ai/api/method/frappe.core.doctype.user.user.update_password',
                formData,
                {
                    headers: { 'Content-Type': 'multipart/form-data' },
                    timeout: 10000, // optional, just in case
                }
            );

            // ✅ Handle success
            if (response.status === 200 || response.data?.message === "Password Updated") {
                setIsSuccess(true);
                showToast("Password updated successfully!", 'success');
            } else {
                setIsSuccess(true);
            ...
            if (axios.isAxiosError(error)) {
                if (error.response) {
                    // ERPNext responded but with an error code
                    const apiError = error.response.data?.message?.error;

                    if (
                        apiError === "The reset password link has been expired" ||
                        apiError === "The reset password link has either been used before or is invalid"
                    ) {
                        errorMessage = "Your password reset link has expired or has already been used. Please request a new reset link.";
                        showRedirect = true;
                ...
                // ✅ Treat "No response" as success
                else if (error.request) {
                    console.warn("No response from server, but assuming success (CORS issue)");
                    setIsSuccess(true);
                    showToast("Password updated successfully!", 'success');
                    return; // ⬅️ stop here so we don’t show an error toast
                }
            } else {
                errorMessage = error instanceof Error ? error.message : errorMessage;
            }
