...
export default function PasswordResetPage() {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            // Use Frappe's API method endpoint instead of raw POST
            const response = await axios.post(
            ...
            if (response.data?.message?.error || response.data?.exc || response.data?.exception) {
                const apiError = response.data.message?.error || 
            ...
            if (axios.isAxiosError(error)) {
                ...
                showToast(typeof apiError === 'string' ? apiError : JSON.stringify(apiError), "error");

                // Show redirect option for expired/invalid links
                const errorString = typeof apiError === 'string' ? apiError : JSON.stringify(apiError);
                if (
                    errorString.toLowerCase().includes("expired") ||
                    errorString.toLowerCase().includes("invalid") ||
                    errorString.toLowerCase().includes("used")
                ) {
                    setShowRedirectOption(true);
                }
            } else {
