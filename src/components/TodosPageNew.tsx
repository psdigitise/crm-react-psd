...
export function TodosPageNew({ onCreateTodo }: TodosPageNewProps) {
    ...
    <div className={`min-h-screen ${theme === 'dark'
      ...
      <div className="p-4 sm:p-6">
        ...
        <div className={`rounded-lg shadow-sm border overflow-hidden ${theme === 'dark'
          ? 'bg-custom-gradient rounded-none border-transparent'
            ...
            <table className="w-full">
              <thead className={`border-b ${theme === 'dark' ? 'bg-purplebg border-b-purplebg' : 'bg-gray-50 border-gray-200'
                ...
                <tr className="divide-x-[1px]">
                  <th className={`px-6 py-3 text-left text-sm font-medium tracking-wider ${theme === 'dark' ? 'text-white' : 'text-gray-500'
                    }`}>
                    Status
                  </th>
                  <th className={`px-6 py-3 text-left text-sm font-medium tracking-wider ${theme === 'dark' ? 'text-white font-semibold' : 'text-gray-500'
                    }`}>
                    Date
                  </th>
                  <th className={`px-6 py-3 text-left text-sm font-medium tracking-wider ${theme === 'dark' ? 'text-white font-semibold' : 'text-gray-500'
                ...
                {filteredTodos.map((todo) => (
                  <tr key={todo.name} className={`transition-colors ${theme === 'dark' ? 'hover:bg-purple-800/20' : ' hover:bg-gray-50'
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-sm bg-transparent text-grey-400 font-semibold ${statusColors[todo.status]}`}>
                        {todo.status}
                      </span>
                    </td>
